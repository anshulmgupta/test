...
public:
    ...
    map< int, pair<int,int> > find_minimum_spaning_tree(int source){
        map<int, int> dist;
        ...
        while(!hp.is_empty()){
            ...
            for(auto child : graph[u->vertex]){
                if(hp.is_in_heap(child.first) && dist[child.first] > child.second){
                    dist[child.first] = child.second;
                    parent[child.first] = {u->vertex, dist[child.first]};
...
void test_case_one(){
    ...
    dgp.add_edge(7, 8, 7);
    int src_vertex = 0;
    auto result = dgp.find_minimum_spaning_tree(src_vertex);
    for(auto d : result){
        if(d.second.first == -1)
            continue;
        cout << "Source Vertex = " << d.second.first << " , Destination Vertex = " << d.first << " , Weight =  " << d.second.second << endl;
    }
    cout << endl;
}
